// storageUtils.js (Version avec IndexedDB Helpers ET LS Helpers Exportés)
...
const DISTRACTIONS_LS_KEY = 'claireAppDistractionsList'; // Clé pour distractions LS
...
function openDB() {
    if (!dbPromise) {
        dbPromise = new Promise((resolve, reject) => {
            ...
            request.onblocked = (e) => { console.warn("Ouverture IDB bloquée.", e); alert("Application bloquée par une autre page ouverte. Fermez les autres onglets Clair·e."); reject("Ouverture bloquée"); };
        });
    }
    return dbPromise;
}
...
async function operateOnStore(storeName, mode = 'readonly', operation) {
    try {
        ...
        return new Promise((resolve, reject) => {
            ...
            // Gérer les différents types de requêtes IDBRequest (qui n'ont pas .then)
             if (!request) { // Cas où l'opération ne retourne pas de requête (rare)
                  resolve(); return;
             }
            request.onsuccess = (event) => resolve(event.target.result);
            request.onerror = (event) => reject(`Erreur requête IDB sur ${storeName}: ${event.target.error}`);
            // Pour les écritures, on attend la fin de la transaction
            if (mode === 'readwrite') {
                 transaction.oncomplete = () => resolve(request.result); // Résoudre avec le résultat de la requête initiale (ex: clé pour put)
                 transaction.onerror = (event) => reject(`Erreur TX ${storeName} ${mode}: ${event.target.error}`);
            } else {
                 // Pour readonly, on peut résoudre dès que la requête réussit
                 // La transaction se fermera automatiquement
            }
        });
...
/** Charge depuis LocalStorage (EXPORTÉE) */
export function loadDataFromLS(key) { // <<< AJOUT 'export'
...
/** Sauvegarde vers LocalStorage (EXPORTÉE) */
export function saveDataToLS(key, data) { // <<< AJOUT 'export'
...
export function getEarnedBadgesFromStorage() { const d = loadDataFromLS(LS_EARNED_BADGES_KEY); return Array.isArray(d) ? d.filter(i => typeof i === 'string') : []; } // Utilise helper interne maintenant exporté
export function saveEarnedBadgesToStorage(badgeIds) { if(!Array.isArray(badgeIds)) return false; return saveDataToLS(LS_EARNED_BADGES_KEY, badgeIds); } // Utilise helper interne maintenant exporté
...
// Distractions (Utilise helpers LS exportés)
...
// ** INDEXEDDB (utilisent les helpers IDB) **
// Journal
